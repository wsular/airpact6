...
!-------------------------------------------------------------
   ...
   allocate( megan_data_orig(nlon,nlat),stat=astat )
...
!-------------------------------------------------------------
   ...
   allocate( megan_data(nlon,nlat),stat=astat )
   allocate( megan_data_org(nlon,nlat),stat=astat )
   


   if( astat /= 0 ) then
     write(*,*) 'discrete_frac: Failed to allocate megan_data; error = ',ierr
     stop 'allocation error'
   endif
   message = 'area_map: Failed to get variable id'
   call handle_ncerr( nf_inq_varid( ncid, trim(varname), varid ), message )
...
!!!
   ...
   write(*,'(''discrete_frac: max model lat ndx = '',i10)') maxval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''discrete_frac: mcells outside data domain = '',1pg22.15)') count( .not. model_area_type(:,:)%has_data )
...
!-------------------------------------------------------------
   ...
   allocate( megan_data_orig(nlon,nlat),stat=astat )
   if( astat /= 0 ) then
     write(*,*) 'area_interp2: Failed to allocate megan_data; error = ',ierr
     stop 'allocation error'
   endif
   message = 'area_map: Failed to get variable id'
   call handle_ncerr( nf_inq_varid( ncid, trim(varname), varid ), message )
...
inside_mcell : &
   ...
   write(*,'(''area_interp2: max model lat ndx = '',i10)') maxval( model_area_type(:,:)%lat_s, &
                                                                      mask=model_area_type(:,:)%has_data )
   write(*,'(''area_interp2: mcells outside data domain = '',1pg22.15)') count( .not. model_area_type(:,:)%has_data )
