...
!----------------------------------------------------
!----allocate pft arrays-----------------------------
!----MEGAN3 has 4 growth forms, 6 canopy type
!----------------------------------------------------
   real, allocatable :: growthform_01(:,:) !BTtree
   real, allocatable :: growthform_02(:,:)
   real, allocatable :: growthform_03(:,:)
   real, allocatable :: growthform_04(:,:) 
   real, allocatable :: growthform_05(:,:) !NTtree
   real, allocatable :: NTfrac(:,:)             ! Needleleaf fraction
   real, allocatable :: Trpfrac(:,:)            ! Trpoical tree fraction
...
!canopy type
   ...
   CHARACTER (LEN=80)  :: NTfilename 
   CHARACTER (LEN=80)  :: NTvarname 
   CHARACTER (LEN=80)  :: NTfillvalue
   
   CHARACTER (LEN=80)  :: TRTfilename 
   CHARACTER (LEN=80)  :: TRTvarname
   CHARACTER (LEN=80)  :: TRTfillvalue
   ...
   INTEGER             :: TRT_FLAG ! 1:Tropical Tree is used 0: No Tropical Tree 
   !INTEGER             :: NT_FLAG ! 1:Tropical Tree is used 0: No Tropical Tree 
   ...
   namelist /cantype/    treefilename,treevarname,treefillvalue, & !hw
                         NTfilename,NTvarname,NTfillvalue, &
                         TRTfilename,TRTvarname,TRTfillvalue, &
...
!-----read growth form----------------------------------------------
      ...
      inpname       = trim(NTfilename)
      varname       = trim(NTvarname)
      read(NTfillvalue,"(f7.0)") missing_value
      print*,"NT_Fillvalue",missing_value
      
      CALL  megan2_bioemiss
      inpname       = trim(TRTfilename)
      varname       = trim(TRTvarname)
...
!---------------------------
   ...
   allocate( growthform_05(ide,jde),stat=astat )
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate growthform_05; error = ',astat
     stop 'allocate failed'
   endif

   allocate( Trpfrac(ide,jde),stat=astat )
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate Trpfrac; error = ',astat
     stop 'allocate failed'
   endif
   Trpfrac = 0.
   growthform_01 = 0.
...
!---------------------------------------------------------------------
   ...
   !use area_mapper, only : area_interp, area_interp2
   use area_mapper_grw, only : area_interp,area_interp2
...
! NTfrac value type is double; others are byte
     if ( varname(:) == trim(TRTvarname))then
      call area_interp2( xedge_megan, yedge_megan, nlon_megan, nlat_megan, int(missing_value,2), &
                        wrk_data, ncid, varname, grid_ndx, new_grid , flip_flag)
     else if ( varname(:) == trim(NTvarname))then
      call area_interp2( xedge_megan, yedge_megan, nlon_megan, nlat_megan, int(missing_value,2), &
                        wrk_data, ncid, varname, grid_ndx, new_grid , flip_flag)
     else
     ...
     elseif( varname(:) == trim(NTvarname)) then
        growthform_05(:,:) = scale_factor * wrk_data(:,:)
     elseif( varname(:) == trim(TRTvarname)) then
        Trpfrac(:,:) = scale_factor * wrk_data(:,:)
     endif
...
!---------------------------------------------------------------------
      ...
      real :: NeedleFrac(ide,jde),BroadFrac(ide,jde),TropicalFrac(ide,jde)


     TreeFrac(:,:) = growthform_01(:,:)  
     ShrubFrac(:,:) = growthform_02(:,:)  
     CropFrac(:,:) = growthform_03(:,:)  
     GrassFrac(:,:) = growthform_04(:,:)  
     !NeedleFrac(:,:) = growthform_05(:,:) 
     !BroadFrac(:,:) = growthform_01(:,:)!growthform_01(:,:) * (1 - (NTfrac(:,:) + Trpfrac(:,:)))
     !TropicalFrac(:,:) = (growthform_01(:,:)+growthform_05(:,:)) * Trpfrac(:,:)
     NeedleFrac(:,:) = growthform_01(:,:)*(1-Trpfrac(:,:))*growthform_05(:,:)
     BroadFrac(:,:) = growthform_01(:,:)*(1-Trpfrac(:,:))*(1-growthform_05(:,:))
     TropicalFrac(:,:) = growthform_01(:,:) * Trpfrac(:,:)


 
      do ilat=1,jde-jds+1
      do ilon=1,ide-ids+1
      if(TreeFrac(ilon,ilat).gt.100) TreeFrac(ilon,ilat)=100.
      if(TreeFrac(ilon,ilat).lt.0.0) TreeFrac(ilon,ilat)=0.0
      if(NeedleFrac(ilon,ilat).gt.100) NeedleFrac(ilon,ilat)=100.
      if(NeedleFrac(ilon,ilat).lt.0.0) NeedleFrac(ilon,ilat)=0.0
      if(BroadFrac(ilon,ilat).gt.100) BroadFrac(ilon,ilat)=100.0
      if(BroadFrac(ilon,ilat).lt.0.0) BroadFrac(ilon,ilat)=0.0
      if(TropicalFrac(ilon,ilat).gt.100.) TropicalFrac(ilon,ilat)=100.0
      if(TropicalFrac(ilon,ilat).lt.0.0) TropicalFrac(ilon,ilat)=0.0
      if(ShrubFrac(ilon,ilat).gt.100.) ShrubFrac(ilon,ilat)=100.0
      if(ShrubFrac(ilon,ilat).lt.0.0) ShrubFrac(ilon,ilat)=0.0
      if(CropFrac(ilon,ilat).gt.100.) CropFrac(ilon,ilat) =100.
      if(CropFrac(ilon,ilat).lt.0.0) CropFrac(ilon,ilat)=0.0
      if(GrassFrac(ilon,ilat).gt.100.) GrassFrac(ilon,ilat)=100.
      if(GrassFrac(ilon,ilat).lt.0.0) GrassFrac(ilon,ilat)=0.0
      end do
      end do


      outpname = trim(out_dir)//'/'//'CT3.csv' !shc added out_dir
      open(unit=99,file=outpname,status='unknown')
      write(99,'(a)')"CID,ICELL,JCELl,NEEDL,TROPI,BROAD,SHRUB,HERB,CROP"
        ...
        do ilon=2,ide-ids
           if ( (ilon.gt.x0) .and. (ilon.le.(ncolsin+x0)) .and.   &
              ...
              write (99, '(3(I,","), 6(F10.4,","))')cell_id,ilon_mcip,ilat_mcip, &
                   NeedleFrac(ilon,ilat),TropicalFrac(ilon,ilat),BroadFrac(ilon,ilat), &
                   ShrubFrac(ilon,ilat),GrassFrac(ilon,ilat),CropFrac(ilon,ilat)
           endif
        end do
     end do
