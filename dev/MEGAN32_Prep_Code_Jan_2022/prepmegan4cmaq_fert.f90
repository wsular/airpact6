...
!--------------------------------------------------------------------
   ...
   integer :: domain
   integer :: mnth, mnth_s, mnth_e, iday
...
!-----read fert----------------------------------------------
      scale_factor  = 1.
    do iday  = 1, 366  !there are 366 days for fert 
         write(str,'(I3.3)')iday
         str = adjustl(str)
         cfert(iday)=trim(str)
         inpname       = 'soil_fert_' // cfert(iday) // '.nc'
         varname       = 'fert'
         missing_value = -999
...
!-----------------------------------------------------------
      write(*,*) 'map_megan2_emissions: Before write_fert'
!write to FERT.csv file---------------------
      CALL write_fert
...
!---------------------------------------------------------------------
   allocate( mfert(ide,jde,366),stat=astat ) 
   if( astat /= 0 ) then
     write(*,*) 'wrf_file: failed to allocate mfert; error = ',astat
...
!---------------------------------------------------------------------
      ...
      if( varname(:4) == 'fert' ) then
      write(*,*)'iday=',iday
!Jiang
        mfert(:,:,iday) = scale_factor * wrk_data(:,:)
      endif
...
!---------------------------------------------------------------------
       ...
       if( ierr /= 0 ) then
         write(message,*) 'Failed to allocate tmp3 for fert; error = ',ierr
         stop 'bio_emiss abort'
       endif
       ...
       do mnth = mnth_s,mnth_e
         do j = jds,jde
           do i = ids,ide
             ...
             if( varname(:4) == 'fert' ) then
!Jiang
        mfert(i,j,iday) = scale_factor * wrk_sum
             endif
...
!---------------------------------------------------------------------
   ...
   if( ret /= nf_noerr ) then
      ...
      subroutine write_fert
!---------------------------------------------------------------------
!       ... write the fert  file
!---------------------------------------------------------------------
!       ... local variables
!---------------------------------------------------------------------
      integer :: cell_id,istep,it,im, iday
      integer :: ilat,ilon
      integer :: ilat_mcip, ilon_mcip !shc
      integer :: dsrad,dtemp
      real :: xfert(ide,jde,366)
      character (len=7) :: fertday(366)
      character (len=7) :: str

        do iday  = 1, 366  !there are 366 days for fert 
         write(str,'(I3.3)') iday
         str = adjustl(str)
         fertday(iday)='FERT'//trim(str)
        end do

      outpname = trim(out_dir)//'/'//'grid_FERT.csv' !shc added out_dir
      open(unit=99,file=outpname,status='unknown')
      write(99,888)"CELL_ID,X,Y,LAT,LONG",fertday
      888 format(a,366(',',a7))
! convert mg/m3 to ng/m3
     xfert(:,:,:)=mfert(:,:,:)*1E+6

!Jiang      do ilat=1,jde-jds+1
!Jiang        do ilon=1,ide-ids+1
!Jiang        do im=1,366 
!Jiang      if(xfert(ilon,ilat,im).gt.24.or.xfert(ilon,ilat,im).lt.0.0)xfert(ilon,ilat,im)=0.0
!Jiang        end do
!Jiang      end do
!Jiang      end do
!-----write new xfert to output
